...
#
# Twelve, and it is the single change that turned the coarse steps from
# unusable to smooth. Worth recording why the arithmetic does not predict it.
#
# A chained run reaches v = sqrt(2ad), so sustaining a feed needs d = v^2 / 2a
# millimetres chained behind it. At $120 = 1500 that is 7.5 mm for 9000 at
# 0.5 mm - 2.5 blocks - and 13.3 mm for 12000 at 1 mm, 3.3 blocks. By that
# measure six was always sufficient and this should change nothing.
#
# It changed everything, so the steady-state figure is the wrong target. Real
# motion dips - transport jitter, block length varying with the commanded feed,
# a hand that pauses to re-grip - and the depth has to cover the dips, not the
# average. Six sat close enough to the requirement that any dip fell through
# it, and the feed followed sqrt down: measured drops to 7146 at 0.5 mm and
# 8958 at 1 mm, which work back to about two blocks chained.
#
# Confirmed rather than inferred, once depth went into the periodic one-liner:
# held reaches 10-13 against this target of 12. So the planner does fill to it,
# and at six it was filling to six and stopping there.
#
# (The sqrt(1500 x d) written further up is sqrt(2ad) for a = 750, so it either
# carries a factor of two or predates $120 doubling along with $110.)
...
# The same bound over ESP-NOW, where the link is quicker and the floor under it
# is correspondingly lower.
#
# The bound has to clear depth plus a worst-case round trip, or it gates the
# fill instead of guarding the run-off. Depth is 0.24 s - PLANNER_TARGET_BLOCKS
# times the tick, whatever the feed or step - and the round trip is what the
# transport costs:
#
#   WiFi at the machine    68-222 ms  ->  floor 0.25-0.46 s  ->  0.5 needed
#   ESP-NOW on the bench    10-60 ms  ->  floor 0.19-0.30 s  ->  0.35 clears
#
# 0.35 was tried on WiFi and pinched, which is exactly what the arithmetic
# above says it should have done. It is not a value that was wrong; it was a
# value that needed a quicker link before it could be right.
#
# Set per transport rather than globally because the WiFi path is the
# documented fallback. A single 0.35 would leave that fallback gating its own
# fill, so falling back would mean falling back to something worse than it was
# - which is not a fallback.
#
# Whichever applies is copied to the scheduler at startup. The bound is a
# property of the link, and this is the only constant here that is.
RUNAHEAD_LIMIT_ESPNOW_S = 0.35
...
class JogScheduler:
    ...
    def __init__(self, encoder, axis="X", step_index=DEFAULT_STEP_INDEX):
        ...
        # Overwritten at startup for a quicker transport. An instance value
        # rather than the constant, because the bound tracks the link's round
        # trip and the link is chosen after this is built.
